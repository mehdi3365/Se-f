from telegram import Update, InlineKeyboardButton, InlineKeyboardMarkup, InlineQueryResultArticle, InputTextMessageContent, InputMediaPhoto, MessageEntity
...
import uuid, emoji, logging, time, json, os, asyncio

TOKEN = "8999838824:AAEJS3gfVj9Nhd21tSVrhKR6rXHy1G6NRKU
"  # توکن ربات
PREMIUM_EMOJI_FILE = "premium_emojis.json"
COMMAND_FILE = "selfbot_commands.json"
SETTINGS_FILE = "settings.json"
REACTION_RESULT_FILE = "reaction_result.json"
PANEL_IMAGE = "https://eramblog.com/img/1713345288_2021441.jpg"  # لینک عکس هلپر اگر نداری فقط عکس رو توی ربات هلپر بفرست خودش تنظیم میشه
PHOTO_FILE = "panel_photo.json"
ADMIN_ID = 8536789126 # ایدی ادمین
MANAGER_BOT_LINK = "https://t.me/Walterselfbot"  # لینک ربات سلف ساز رو اینجا بذار

def find_emoji_position(text: str):
    for item in emoji.emoji_list(text):
        emoji_text = item["emoji"]
        start = item["match_start"]
        end = item["match_end"]

        return emoji_text, start, end

    return None, None, None

def utf16_offset(text: str):
...
def load_premium_emojis():
    try:
        ...
        with open(PREMIUM_EMOJI_FILE, "r", encoding="utf-8") as f:
            return json.load(f)

    except Exception as e:
        print(f"Premium emoji load error: {e}")
        return {}
...
async def handle_premium_emoji_inline_query(
    update: Update,
    context: ContextTypes.DEFAULT_TYPE
):
    try:
        logger.info("🚀 وارد handle_premium_emoji_inline_query شد")

        query = update.inline_query.query

        logger.info(f"📝 کوئری دریافتی: {query!r}")

        if not query.startswith("pe"):
            return
        rest = query[2:]
        ...
        first_emoji = found_emojis[0]
        info = premium_data.get(first_emoji)

        if not info:
            logger.warning(
                f"❌ ایموجی پریمیوم پیدا نشد: {first_emoji}"
            )
        ...
        if isinstance(info, str):
            custom_emoji_id = info

        elif isinstance(info, dict):
            custom_emoji_id = str(
                info.get("custom_emoji_id", "")
            )
        ...
        if not custom_emoji_id:
            logger.warning("❌ custom_emoji_id خالی است")
        ...
        logger.info(
            f"📝 متن نهایی بدون تغییر: {full_text!r}"
        )
        result_id = f"premium_{custom_emoji_id}_{uuid.uuid4().hex[:8]}"
        ...
        title = f"✨ {len(found_emojis)} ایموجی"

        results = [
            InlineQueryResultArticle(
                id=result_id,
                title=title,
                description=f"{len(found_emojis)} ایموجی پرمیوم می‌شوند",
        ...
        logger.info("✅ پاسخ اینلاین ارسال شد")

    except Exception as e:
        logger.exception(
            f"❌ خطا در handle_premium_emoji_inline_query: {e}"
        )
...
async def handle_chosen_inline_result(
    ...
    try:
        logger.info(
            "🔄 شروع handle_chosen_inline_result"
        )
        ...
        if not inline_message_id:
            logger.warning(
                "⚠️ inline_message_id موجود نیست"
            )
            return

        if not result_id.startswith("premium_"):
            return
        ...
        if len(parts) < 3:
            logger.warning(
                f"❌ result_id نامعتبر: {result_id}"
            )
            return
        custom_emoji_id = "_".join(parts[1:-1])

        if not custom_emoji_id:
            logger.warning(
                "❌ custom_emoji_id خالی است"
            )
            return

        logger.info(
            f"🔍 custom_emoji_id: {custom_emoji_id}"
        )

        query_text = result.query

        logger.info(
            f"📝 query اصلی: {query_text!r}"
        )

        if not query_text.startswith("pe"):
            return
        rest = query_text[2:]
        ...
        for emoji_char in premium_data.keys():
            if emoji_char in rest:
                found_emojis.append(emoji_char)
                start = rest.find(emoji_char)
                end = start + len(emoji_char)
                emoji_positions.append({
                    'emoji': emoji_char,
                    'start': start,
                    'end': end,
                    'custom_id': premium_data[emoji_char]
                })

        if not found_emojis:
            logger.warning(
                "❌ هیچ ایموجی ثبت شده‌ای در متن پیدا نشد"
            )
            return
        ...
        for emoji_data in emoji_positions:
            emoji_offset = utf16_offset(
                rest[:emoji_data['start']]
            )
            
            emoji_length = utf16_offset(
                rest[emoji_data['start']:emoji_data['end']]
            )
            info = emoji_data['custom_id']
            if isinstance(info, str):
                custom_id = info
            elif isinstance(info, dict):
                custom_id = str(info.get("custom_emoji_id", ""))
            else:
                custom_id = ""
            ...
            if custom_id:
                ...
                logger.info(
                    f"📍 ایموجی {emoji_data['emoji']!r} "
                    f"offset={emoji_offset} length={emoji_length}"
                )

        if not entities:
            logger.warning("❌ هیچ Entity ساخته نشد")
            return

        full_text = rest

        logger.info(
            f"📝 متن برای ادیت: {full_text!r}"
        )
        ...
        try:
            ...
            logger.info(
                f"✅ {len(entities)} Premium Emoji اعمال شد: "
                f"{full_text!r}"
            )

        except Exception as e:
            logger.exception(
                f"❌ خطا در edit_message_text: {e}"
            )

    except Exception as e:
        logger.exception(
            f"❌ خطا در handle_chosen_inline_result: {e}"
        )
...
def main():
    ...
    app.add_handler(InlineQueryHandler(handle_premium_emoji_inline_query, pattern=r"^pe"))
    app.add_handler(ChosenInlineResultHandler(handle_chosen_inline_result))
    app.add_handler(InlineQueryHandler(handle_inline_query, pattern=r"^panel$"))
    app.add_handler(CallbackQueryHandler(handle_callback))
    app.add_error_handler(error_handler)
    app.add_error_handler(lambda u, c: logger.error(f"❌ خطا: {c.error}"))
    
    print("🤖 ربات هلپر اجرا شد")
    print("📌 pe 😂 سلام -> ایموجی پریمیوم")
    print("📌 panel -> پنل مدیریت")
    app.run_polling()
